...
async function addProductToFiles() {
  let connection;
  try {
    ...
    console.log('Ajout du champ product_id à la table files...');

    // Vérifier si la colonne existe déjà
    const [columns] = await connection.execute(`
      SELECT COLUMN_NAME 
      FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'files' AND COLUMN_NAME = 'product_id'
    `, [process.env.DB_NAME]);
