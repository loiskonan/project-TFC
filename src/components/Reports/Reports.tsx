...
import { useCompleteStats } from '../../hooks/useCompleteStats';
import { useCompleteFilesStats } from '../../hooks/useCompleteFilesStats';
import StatsCard from '../Dashboard/StatsCard';

const Reports: React.FC = () => {
  const { currentUser } = useAuth();
  const { stats: sendStats, loading: sendLoading, error: sendError } = useCompleteStats();
  const { stats: receiveStats, loading: receiveLoading, error: receiveError } = useCompleteFilesStats();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">
          {isAdmin ? 'Rapports - Administration' : 
           isNsiaVie ? 'Rapports - NSIA Vie' : 
           `Rapports - ${currentUser?.banque || 'Ma banque'}`}
        </h3>
        <p className="text-gray-600">
          Vue d'ensemble complète de toutes les statistiques 
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h4 className="text-lg font-semibold text-gray-900 mb-6 flex items-center">
          ...
          {isAdmin || isNsiaVie ? 'Statistiques des Envois' : 'Mes Statistiques d\'Envoi'}
        </h4>
        ...
        {sendStats ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <StatsCard
                title={isAdmin ? "Total fichiers envoyés" : isNsiaVie ? "Total fichiers NSIA envoyés" : "Mes fichiers envoyés"}
              ...
              />
              <StatsCard
                title={isAdmin ? "Stockage total envoyé" : isNsiaVie ? "Stockage NSIA total envoyé" : "Mon stockage total envoyé"}
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div className="text-sm text-blue-800">Total envoyés</div>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">{sendStats.filesByStatus?.downloaded || 0}</div>
                    <div className="text-sm text-purple-800">Téléchargés</div>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h4 className="text-lg font-semibold text-gray-900 mb-6 flex items-center">
          ...
          {isAdmin || isNsiaVie ? 'Statistiques des Réceptions' : 'Mes Statistiques de Réception'}
        </h4>
        ...
        {receiveStats ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <StatsCard
                title={isAdmin ? "Total fichiers reçus" : isNsiaVie ? "Total fichiers NSIA reçus" : "Mes fichiers reçus"}
              ...
              />
              <StatsCard
                title={isAdmin ? "Stockage total reçu" : isNsiaVie ? "Stockage NSIA total reçu" : "Mon stockage total reçu"}
